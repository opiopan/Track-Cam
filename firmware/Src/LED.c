...
static LEDSeqUnit LEDSeqShortBlink[] = {LED_SEQ_UNIT(3, 1), LED_SEQ_UNIT(3,0), LED_SEQ_END};
static LEDSeqUnit LEDSeqBlip[] = {LED_SEQ_UNIT(4, 1), LED_SEQ_UNIT(20, 0), LED_SEQ_END};
static LEDSeqUnit LEDSeqDoubleBlip[] =
	{LED_SEQ_UNIT(2, 1), LED_SEQ_UNIT(2, 0), LED_SEQ_UNIT(2, 1), LED_SEQ_UNIT(18, 0), LED_SEQ_END};
 ...
 *-------------------------------------------------------------*/
int initLED(LEDHandle* handle, TIM_HandleTypeDef* timer,
...
void scheduleLED(LEDHandle* handle)
{
	...
	if (context->needToUpdateUserSeq){
		context->userSeq = context->nextUserSeq[context->needToUpdateUserSeq - 1];
	...
	for (level = 0; level < LED_SEQ_LEVEL_NUM; level++){
		if (context->needToUpdateConfig[level]){
			volatile LEDConfig* config =
					&context->nextConfig[level][context->needToUpdateConfig[level] - 1];
			int led;
			for (led = 0; led < LED_NUM; led++){
				...
				if (type == LED_SEQ_TYPE_NA){
					...
					needToReset = 1;
				}else if (type == LED_SEQ_TYPE_USER && context->userSeq.sequence[led][0]){
					...
					context->seqContext[level][led].sequence = seqTable[type].conf[led].seq;
					context->seqContext[level][led].stage = seqTable[type].conf[led].offset;
					context->seqContext[level][led].count = -1;
				}
			}
	...
	for (level = 0; level < LED_SEQ_LEVEL_NUM; level++){
		int led;
		for (led = 0; led < LED_NUM; led++){
			volatile LEDSeqContext* seqContext = &context->seqContext[level][led];
			if (seqContext->sequence){
				needToReflect[led] = 0;
				seqContext->count++;
				int16_t duration = LED_SEQ_UNIT_DURATION(seqContext->sequence[seqContext->stage]);
				...
				if (seqContext->count == 0 || needToReset){
					needToReflect[led] = 1;
					state[led] = LED_SEQ_UNIT_MODE(seqContext->sequence[seqContext->stage]);
				}
			}
		}
	}
	...
	for (led = 0; led < LED_NUM; led++){
		if (needToReflect[led]){
			  HAL_GPIO_WritePin(context->gpio[led].ch, context->gpio[led].pin, state[led]);
		}
	}
}
