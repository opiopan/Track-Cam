 ...
 * Command handler
 *-------------------------------------------------------------*/
static int getServoMode(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
...
static int setServoMode(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int alen, uint8_t* respBuf, int rblen)
{
    if (alen != 0) {
        return 0;
    ...
    if (lastMode == SERVO_IDLE && mode != SERVO_IDLE) {
        LED_CONFIG(handle->led, LED_SEQ_LEVEL_MASTER).type[LED_BLUE] =
                LED_SEQ_TYPE_SERVO_RUNNING;
        LED_CONFIG(handle->led, LED_SEQ_LEVEL_MASTER).type[LED_RED] =
                LED_SEQ_TYPE_SERVO_RUNNING;
...
static int getServoPos(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int alen, uint8_t* respBuf, int rblen)
{
    if (alen != 0) {
        return 0;
...
static int getServoPosRaw(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int alen, uint8_t* respBuf, int rblen)
{
    if (alen != 0) {
        return 0;
...
static int setServoDeltaTheta(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int alen, uint8_t* respBuf, int rblen)
{
...
static int setServoDuty(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int alen, uint8_t* respBuf, int rblen)
{
...
static int setServoThetaDuty(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int alen, uint8_t* respBuf, int rblen)
{
...
static int setLEDMode(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int alen, uint8_t* respBuf, int rblen)
{
...
int processHostCommand(HostCommHandle* handle, uint8_t cmd, uint8_t* arg,
        int argLength, uint8_t* outBuf, int outBufLength)
{
    uint8_t idx = dispatchIdx[cmd];
    if (idx != CMD_INVALID) {
        return dispatch[idx].func(handle, cmd, arg, argLength, outBuf,
                outBufLength);
    }
    return 0;
}
